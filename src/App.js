...
function App() {
  ...
  const BASE_URL = "https://backend-sudimampir-production.up.railway.app";
  ...
  const confirmCloseShift = async () => {
    try {
      const response = await fetch(`${BASE_URL}/api/close-shift`, {
          method: "POST",
          headers: {
  ...
  const handleSubmitMenu = async (e) => {
    ...
    try {
      if (editingId) {
        const response = await fetch(`${BASE_URL}/api/menu/${editingId}`, {
          method: 'PUT',
          headers: {
                ...
                <div className="menu-list">
                  {filteredMenu.map(item => (
                    <div key={item.id} className="menu-card">
                      <img src={`${BASE_URL}${item.image}`} alt={item.name} className="menu-img" />
              ...
              <div className="kasir-grid">
                <div className="cart-card">
                  ...
                  <form onSubmit={handleSubmitMenu} style={{ marginTop: '15px' }}>
                    ...
                    {newMenu.image && <img src={`${BASE_URL}${newMenu.image}`} alt="preview" style={{ width:100, height:100, objectFit:"cover", borderRadius:8 }} />}
                  ...
                  {menuList.map(item => (
                    <div key={item.id} className="cart-item" style={{ background: 'white', padding: '15px', borderRadius: '8px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <img src={`${BASE_URL}${item.image}`} alt="thumb" style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '8px' }}/>
                        <div>
